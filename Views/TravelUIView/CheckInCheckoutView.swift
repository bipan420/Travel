...
class CheckInCheckOutView : UIView{
    ...
    let dayLabel = UILabel(text: "14", font: .systemFont(ofSize: 44, weight: .bold), textColor: .black, textAlignment: .center, numberOfLines: 1)
    ...
    init(type: Int) {
        ...
        stack(
            ...
            hstack(dayLabel,
                   stack(weekDayLabel,monthLabel).withMargins(UIEdgeInsets(top: 0, left: 5, bottom: 0, right: 0)),
                UIView()
            ).withMargins(UIEdgeInsets(top: 20, left: 10, bottom: 24, right: 0)),
            UIView()
        )
